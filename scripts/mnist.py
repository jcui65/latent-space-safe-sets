...
class ClusterStateManager:
    ...
    def signal_handler(self, signal, frame):
        print("Received signal [", signal, "]")#control c is 2
        self.external_exit = signal

    def timer_handler(self, signal, frame):
        print("Received alarm [", signal, "]")#it is 14
...
def save(network, optimizer, epoch):#this is not a function in the above class!
...
csm = ClusterStateManager(90)#ClusterStateManager(30)#ClusterStateManager(120)#ClusterStateManager(240)#
...
def train(model, device, train_loader, optimizer, epoch, log_interval):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if csm.should_exit():
            return
        #if no need to exit, then just go on training!
        data, target = data.to(device), target.to(device)#to gpu device!
        optimizer.zero_grad()
        output = model(data)
...
def main():
    ...
    for epoch in range(cur_epoch, epochs + 1):
        ...
        if csm.should_exit():#it is as if the last one from the train function!
            break
    ...
    print("Exiting with exit code", csm.get_exit_code())#finally, when you finished training
