...
import os
import logging
from tqdm import tqdm,trange#mainly for showing the progress bar
import numpy as np
import pprint
...
if __name__ == '__main__':
    ...
    for m in range(repeattimes):
        ...
        if params['hasbeenpreempted']=='no':#os.path.exists(datasave_dir):#this means that, not yet been preempted, should start from scratch
            ...
            logdirold='outputs/2023-05-12/22-18-44/2'
            real_dir = os.path.join('', logdirold,'datarun')  #
            num=70#210#300
            trajectories2 += utils.load_trajectories_latent(num, file=real_dir)#now you have 50+50=100 pieces of trajs each containing 100 time step
            ...
            for j in range(traj_per_update):#default 10 in spb
                ...
                for k in trange(params['horizon']):#default 100 in spb#This is MPC
                    ...
                    if params['push_cbf_strategy']==1:
                        transition = {'obs': obs, 'action': tuple(action64), 'reward': reward,
                ...
                utils.save_trajectory_latent(transitions, datasave_dir, i*traj_per_update+j,encoder,params['mean'])#
                #utils.save_trajectory_latent(transitions, datasave_dir, i*traj_per_update+j)#
