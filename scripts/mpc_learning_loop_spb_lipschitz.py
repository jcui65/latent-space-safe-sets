...
from latentsafesets.utils.teacher import ConstraintTeacher, OutburstPushTeacher
import sympy
...
if __name__ == '__main__':
    ...
    for m in range(repeattimes):
        ...
        f.write('What I want to write: Lipschitz constant calculation for spb, %s'%(params['quote']))
        f.close()
        ...
        for i in range(num_updates):#default 25 in spb
            ...
            teacher=ConstraintTeacher(env,noisy=False)#OutburstPushTeacher(env,noisy=False)#ReacherConstraintdense2Teacher(env,noisy=False)
            ...
            datasave_dir = os.path.join(logdir, "SimplePointBotConstraints")#create the corresponding folder!
            ...
            for j in range(traj_per_update):#default 10 in spb
                ...
                obs=env.reset(random_start=True)#self.random_start)#obs
                ...
                for k in trange(params['horizon']):#default 100 in spb#This is MPC
                    ...
                    state=env.state#spb has state as its member function!#env.position#env.current_state#
                    ...
                    action = teacher._expert_control_lipschitz(state, k).astype(np.float64)#here the k is the horizon
