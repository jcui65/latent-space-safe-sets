...
def main():
    ...
    #save_folder = os.path.join('data_images_relative', env_name)  # data_images/spb
    ...
    for trajectory in tqdm(demo_trajectories):#show progress bar! 100 trajectories!
        for frame in trajectory:#100 frames in one trajectory!
            if frame_stack == 1:
                ...
                #imr = Image.fromarray(frame['obs_relative'].transpose((1, 2, 0)))  # maybe channel last
                #imr.save(os.path.join(save_folder, 'relative_%d.png' % i))  # save 3 channel obs image png
            else:
                for j in range(frame_stack):
                    im = Image.fromarray(frame['obs'][j].transpose((1, 2, 0)))
                    im.save(os.path.join(save_folder, '%d_%d.png' % (i, j)))
                    #imr = Image.fromarray(frame['obs_relative'][j].transpose((1, 2, 0)))
                    #imr.save(os.path.join(save_folder, 'relative_%d_%d.png' % (i, j)))
            i += 1
