...
def simple_plot4(data, data2, data3,data4=None,std=None,std2=None,std3=None,std4=None, title=None, show=False, file=None, ylabel=None, xlabel=None, log=False, nonreward=False):
    ...
    if std3 is not None:
        ...
        plt.fill_between(xs3, lower3, upper3, alpha=0.3)
    ...
    if std4 is not None:
        ...
        plt.fill_between(xs4, lower4, upper4, alpha=0.3)
...
def visualize_cbfdotconly(obs, onezero, file, env=None):
    if issubclass(type(env), SimplePointBot):
        ...
        spbu.evaluate_cbfdotconly_func(onezero, env, file=file, skip=1, action=(0, 0))  # 61 in spb_utils
        return
    #print('entering the other one!')
    ss = onezero.prob(obs, already_embedded=True).squeeze()
    obs = ptu.to_numpy(onezero.encoder.decode(ptu.torchify(obs)))
