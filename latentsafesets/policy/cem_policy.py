...
from latentsafesets.modules import VanillaVAE, PETSDynamics, ValueFunction, ConstraintEstimator, \
    GoalIndicator, CBFdotEstimator,CBFdotEstimatorlatentplana#they are all there
...
class CEMSafeSetPolicy(Policy):
    ...
    def actcbfdsquarelatentplanaexpensive(self, obs,state,tp,fp,fn,tn,tpc,fpc,fnc,tnc):#some intermediate step that the cbf dot part still requires states rather than latent states
        """
        Returns the action that this controller would take at time t given observation obs.
        Arguments:obs: The current observation. Cannot be a batch
        Returns: An action (and possibly the predicted cost)
