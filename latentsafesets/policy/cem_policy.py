...
class CEMSafeSetPolicy(Policy):
    def __init__(self, env: gym.Env,
        ...
        self.cbfd_adaptive = params['cbfd_adaptive']
        self.cbfd_iterations=params['cbfd_iterations']
        #self.cbfd_adaptive=True#False#
    ...
    def actcbfdsquarelatentplanai(self, obs,state,tp,fp,fn,tn,tpc,fpc,fnc,tnc,i):#some intermediate step that the cbf dot part still requires states rather than latent states
        """
        Returns the action that this controller would take at time t given observation obs.
        Arguments:obs: The current observation. Cannot be a batch
        Returns: An action (and possibly the predicted cost)
        ...
        reset_count = 0#
        act_ss_thresh = self.safe_set_thresh#initially 0.8
        if self.cbfd_adaptive:
            ci=self.cbfd_iterations
            #act_cbfd_thresh = (self.cbfd_thresh+min(i/100,1-self.cbfd_thresh))  # initially 0.8
            act_cbfd_thresh = (self.cbfd_thresh + min(i / ci, 1 - self.cbfd_thresh))  # initially 0.8
        else:
            act_cbfd_thresh=self.cbfd_thresh#initially 0.8
        while itr < self.max_iters:#5
            if itr == 0:
    ...
    def actcbfdsquarelatentplanaicbfqp(self, obs,state,tp,fp,fn,tn,tpc,fpc,fnc,tnc,i):#some intermediate step that the cbf dot part still requires states rather than latent states
        """
        Returns the action that this controller would take at time t given observation obs.
        Arguments:obs: The current observation. Cannot be a batch
        Returns: An action (and possibly the predicted cost)
        ...
        # encode observation:
        obs = ptu.torchify(obs).reshape(1, *self.d_obs)#just some data processing
        emb = self.encoder.encode(obs)#in latent space now!#z_t
        embrepeat=emb.repeat(self.popsize,self.plan_hor,1)#emb.repeat(1000,5,1), with new shape (1000,5,32)#1000 and 5 should subject to change#print('embrepeat.shape',embrepeat.shape)
        embrepeat20 = emb.repeat(self.n_particles, self.popsize, 1, 1)  #with new shape (20,1000,1,32)#
        itr = 0#
        reset_count = 0#
        ...
        action = actions_sorted[-1][0]#the best one
        #print('action', action)
        #log.info('action x before cbf-qp:%f, action y before cbf qp:%f' % (action[0], action[1]))
        # now let's try to do CBF-QP with learned CBF using scipy!
        ...
        actiondiff=actioncbfqp-ptu.to_numpy(action)
        if np.linalg.norm(actiondiff)>1e-6:
            log.info('action diff x:%f, action diff y:%f' % (actiondiff[0], actiondiff[1]))
        #the ground truth is by assuming you know the states and do really CBF-QP!

        return actioncbfqp, tp,fp,fn,tn,tpc,fpc,fnc,tnc#action.detach().cpu().numpy(), tp,fp,fn,tn,tpc,fpc,fnc,tnc#
